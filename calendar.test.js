...
test('historical windows exclude today exactly', () => {
  assert.deepEqual(dateWindow('past7', fixedNow, 'Europe/Brussels'), {
    start: '2026-08-16', end: '2026-08-22', kind: 'past7', today: '2026-08-23', allowPast: true
  });
  assert.deepEqual(dateWindow('month', fixedNow, 'Europe/Brussels'), {
    start: '2026-08-01', end: '2026-08-22', kind: 'month', today: '2026-08-23', allowPast: true, empty: false
  });
});

test('historical windows allow genuine past release dates but still exclude today', () => {
  const past7 = dateWindow('past7', fixedNow, 'Europe/Brussels');
  assert.ok(movieDetailsToMeta(movieDetails({ digitalDate: '2026-08-22' }), 'Netflix', past7).meta);
  assert.ok(movieDetailsToMeta(movieDetails({ digitalDate: '2026-08-16' }), 'Netflix', past7).meta);
  assert.equal(movieDetailsToMeta(movieDetails({ digitalDate: '2026-08-23' }), 'Netflix', past7).reason, 'outside-window');
  assert.equal(movieDetailsToMeta(movieDetails({ digitalDate: '2026-08-15' }), 'Netflix', past7).reason, 'past');
});

test('month history is empty on the first day of a month instead of leaking previous month', () => {
  assert.deepEqual(dateWindow('month', new Date('2026-09-01T10:00:00Z'), 'Europe/Brussels'), {
    start: '2026-09-01', end: '2026-08-31', kind: 'month', today: '2026-09-01', allowPast: true, empty: true
  });
});
...
test('manifest is catalog-first with ten fixed-period native Home rows', () => {
  const manifest = api._internals.buildManifest('https://example.com');
  assert.equal(manifest.version, '1.0.0');
  assert.equal(manifest.id, 'com.nuvio.usareleases.catalog');
  assert.equal(manifest.name, 'Nuvio USA Releases Catalog');
  assert.equal(manifest.catalogs.length, 10);
  assert.equal(manifest.catalogs[0].id, 'usa-releases-today-movie');
  assert.equal(manifest.catalogs[0].name, 'Aujourd’hui • Films');
  assert.equal(manifest.catalogs[1].id, 'usa-releases-today-series');
  assert.equal(manifest.catalogs[1].name, 'Aujourd’hui • Séries');
  assert.ok(manifest.catalogs.some((c) => c.id === 'usa-releases-month-movie'));
  assert.ok(manifest.catalogs.some((c) => c.id === 'usa-releases-past7-series'));
  assert.ok(manifest.catalogs.every((c) => c.showInHome === true));
  assert.ok(manifest.catalogs.every((c) => !c.extra));
});

test('Explore period parser maps Nuvio genre extra to calendar windows', () => {
  assert.equal(api._internals.periodFromExtra(undefined), 'today');
  assert.equal(api._internals.periodFromExtra('Ce mois'), 'month');
  assert.equal(api._internals.periodFromExtra('7 derniers jours'), 'past7');
  assert.equal(api._internals.periodFromExtra('Aujourd’hui'), 'today');
  assert.equal(api._internals.periodFromExtra("Aujourd'hui"), 'today');
  assert.equal(api._internals.periodFromExtra('Demain'), 'tomorrow');
  assert.equal(api._internals.periodFromExtra('7 prochains jours'), 'week');
  assert.deepEqual(
    api._internals.parseCatalogExtraSegment('genre=7%20prochains%20jours&skip=100'),
    { genre: '7 prochains jours', skip: '100' }
  );
});
...
test('streaming discover remains US flatrate and does not add timezone conversion to TV dates', () => {
  const catalog = api._internals.CATALOGS['calendar-netflix-series'];
...
test('historical windows also work for streaming series, TV USA and anime', () => {
  const past7 = dateWindow('past7', fixedNow, 'Europe/Brussels');

  const webEpisode = tvmazeWebEpisode({
    airdate: '2026-08-20',
    airstamp: '2026-08-20T07:00:00Z',
    provider: 'Netflix',
    localCountry: null
  });
  const webDetails = seriesDetails({ id: 120, name: 'Streaming Show' });
  const web = api._internals.tvmazeStreamingEpisodeToMeta(
    webEpisode,
    webDetails,
    { label: 'Netflix', ids: netflixIds, aliases: ['Netflix'] },
    'Europe/Brussels',
    past7
  );
  assert.ok(web.meta);
  assert.equal(web.meta.released, '2026-08-20');

  const broadcastEpisode = tvmazeEpisode({
    airstamp: '2026-08-20T21:00:00-04:00',
    airdate: '2026-08-20'
  });
  const broadcast = api._internals.tvmazeBroadcastToMeta(
    broadcastEpisode,
    'Europe/Brussels',
    past7,
    fixedNow
  ...
  assert.equal(broadcast.meta.released, '2026-08-21');

  const anime = animeSchedule({
    airingAt: Math.floor(new Date('2026-08-20T15:30:00Z').getTime() / 1000)
  });
  const animeResult = api._internals.animeScheduleToMeta(
    anime,
    seriesDetails({ id: 700, name: 'Exact Anime' }),
    'Europe/Brussels',
    past7
  );
  assert.ok(animeResult.meta);
  assert.equal(animeResult.meta.released, '2026-08-20');
});
...
test('buildStreamingCatalog excludes wrong provider after local revalidation', async () => withTmdbToken(async () => {
  ...
  try {
    const result = await api._internals.buildStreamingCatalog({
      catalog: api._internals.CATALOGS['calendar-netflix-movie'],
...
test('buildTvBroadcastCatalog keeps real viewer time and ignores other source dates', async () => withTmdbToken(async () => {
  ...
  try {
    const result = await api._internals.buildTvBroadcastCatalog({
      catalog: api._internals.CATALOGS['calendar-tv-usa-series'],
      period: 'week',
      timeZone: 'Europe/Brussels',
...
test('buildAnimeCatalog maps exact anime to TMDb and returns converted airing', async () => withTmdbToken(async () => {
  ...
  try {
    const result = await api._internals.buildAnimeCatalog({
      catalog: api._internals.CATALOGS['calendar-anime-series'],
      timeZone: 'Europe/Brussels',
...
test('catalog release cards use a safe SVG endpoint without changing detail IDs', () => {
  assert.equal(api._internals.isAllowedPosterSource('https://image.tmdb.org/t/p/w500/test.jpg'), true);
  assert.equal(api._internals.isAllowedPosterSource('https://evil.example/poster.jpg'), false);
  const meta = {
    id: 'tt1234567', type: 'series', name: 'Network Show',
    poster: 'https://image.tmdb.org/t/p/w500/test.jpg',
    releaseInfo: 'S01E02 • Aujourd’hui • 03:00'
  };
  const url = api._internals.calendarCardUrl(
    'https://calendar.example', meta, { name: 'TV USA', type: 'series' }, 'Europe/Brussels'
  );
  assert.match(url, /^https:\/\/calendar\.example\/release-card\.svg\?/);
  assert.match(url, /TV\+USA|TV%20USA/);
  assert.match(url, /Europe%2FBrussels/);
  assert.equal(meta.id, 'tt1234567');
});

test('calendar card SVG embeds native calendar labels and escapes text', () => {
  const svg = api._internals.calendarCardSvg({
    imageDataUri: 'data:image/png;base64,AA==',
    title: 'Show & Friends',
    provider: 'Netflix',
    info: 'S02E04 • Aujourd’hui • 09:00',
    type: 'series'
  });
  assert.match(svg, /image\/png;base64,AA==/);
  assert.match(svg, /Show &amp; Friends/);
  assert.match(svg, /Netflix/);
  assert.match(svg, /Aujourd’hui/);
  assert.match(svg, /SÉRIE/);
});


test('global merge combines provider badges only for the exact same calendar event', () => {
  const merge = api._internals.publicGlobalSortAndMerge;
  const rows = merge([
    { id: 'tt1', type: 'series', name: 'Show', released: '2026-08-23', releaseInfo: 'S01E02 • Aujourd’hui • 03:00', calendarProviders: ['Netflix'] },
    { id: 'tt1', type: 'series', name: 'Show', released: '2026-08-23', releaseInfo: 'S01E02 • Aujourd’hui • 03:00', calendarProviders: ['Prime Video'] },
    { id: 'tt1', type: 'series', name: 'Show', released: '2026-08-24', releaseInfo: 'S01E03 • Demain • 03:00', calendarProviders: ['Netflix'] }
  ]);
  assert.equal(rows.length, 2);
  assert.deepEqual(rows[0].calendarProviders.sort(), ['Netflix', 'Prime Video']);
  assert.deepEqual(rows[1].calendarProviders, ['Netflix']);
});

test('calendar card renders distinctive multiple provider badges', () => {
  const svg = api._internals.calendarCardSvg({
    title: 'Global Show',
    badges: ['Netflix', 'Prime Video', 'Disney+'],
    info: 'S01E02 • Aujourd’hui • 03:00',
    type: 'series'
  });
  assert.match(svg, /Netflix/);
  assert.match(svg, /Prime Video/);
  assert.match(svg, /Disney\+/);
  assert.match(svg, /#e50914/);
  assert.match(svg, /#00a8e1/);
});
