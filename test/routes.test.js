...
test('collections route is platform parents -> Series/Films -> five periods -> months',async()=>{
  ...
  try{
    ...
    assert.equal(p[0].folders[0].coverImageUrl,'https://archives.example/platform-category-card.svg?provider=netflix&category=series&v=153');
    assert.equal(p[0].folders[1].titleLogoUrl,'https://archives.example/platform-logo?provider=netflix&type=movie&v=153');
  }finally{delete process.env.NUVIO_NOW_OVERRIDE}
});
